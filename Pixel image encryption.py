...
def open_img():
    ...
    if x:
        img = Image.open(x)
        eimg = img
        img = img.resize((300, 300), Image.ANTIALIAS)  # Resize image to display
...
def en_fun(x):
    ...
    if image_input is not None:
        ...
        imge = imge.resize((300, 300), Image.ANTIALIAS)  # Resize encrypted image to display
...
def de_fun():
    ...
    if image_encrypted is not None and key is not None:
        ...
        imgd = imgd.resize((300, 300), Image.ANTIALIAS)  # Resize decrypted image to display
